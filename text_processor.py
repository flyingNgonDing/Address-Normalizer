...
FIXED: Proper handling of "thị trấn" and other administrative units
...
def chuan_hoa(text):
    """
    Chuẩn hóa text địa chỉ - FIXED: Remove "thị trấn" prefix to match mapping data
    ...
    # Bước 3: FIXED - Loại bỏ các đơn vị hành chính đầy đủ (để match với mapping data)
    # Mapping data chỉ chứa tên thuần túy, không có "thị trấn", "thành phố", etc.
    admin_full_removals = [
        r'\bthị\s+trấn\s+',     # "thị trấn " -> remove completely
        r'\bthành\s+phố\s+',    # "thành phố " -> remove completely  
        r'\bthị\s+xã\s+',       # "thị xã " -> remove completely
        r'\bxã\s+',             # "xã " -> remove
        r'\bphường\s+',         # "phường " -> remove
        r'\bquận\s+',           # "quận " -> remove
        r'\bhuyện\s+',          # "huyện " -> remove
        r'\btỉnh\s+',           # "tỉnh " -> remove
        r'\bthôn\s+',           # "thôn " -> remove
        r'\bấp\s+',             # "ấp " -> remove
        r'\bkhu\s+phố\s+',      # "khu phố " -> remove
        r'\btổ\s+dân\s+phố\s+', # "tổ dân phố " -> remove
        r'\bkhu\s+vực\s+',      # "khu vực " -> remove
    ]
    
    for pattern in admin_full_removals:
        text = re.sub(pattern, '', text, flags=re.IGNORECASE)
    
    # Bước 4: Xóa trực tiếp các viết tắt hành chính
    for admin_pattern in REGEX_PATTERNS['admin_removals']:
        text = admin_pattern.sub('', text)
    
    # Bước 5: Áp dụng mapping viết tắt cho địa danh cụ thể
    for pattern, replacement in VIETTAT_MAP.items():
        text = re.sub(pattern, replacement, text)
    
    # Bước 6: Xử lý số thứ tự trong tên
    text = re.sub(r'\b(\d+)(st|nd|rd|th)\b', r'\1', text)
    
    # Bước 7: Chuẩn hóa Unicode
    text = unicodedata.normalize('NFKD', text)
    text = ''.join(c for c in text if not unicodedata.combining(c))
    
    # Bước 8: Xử lý số (loại bỏ số 0 đầu)
    text = REGEX_PATTERNS['remove_zeros'].sub(r'\1', text)
    
    # Bước 9: Chuẩn hóa dấu và khoảng trắng
    text = text.replace('-', ' ')
    text = REGEX_PATTERNS['special_chars'].sub(' ', text)
    text = REGEX_PATTERNS['whitespace'].sub(' ', text)
    
    # Bước 10: Làm sạch cuối cùng
    text = text.strip()
    
    # Bước 11: Loại bỏ các từ đơn lẻ không có nghĩa (cuối cùng)
    words = text.split()
    meaningful_words = []
    ...
    for word in words:
        ...
        # 1. Số
        # 2. Từ dài >= 2 ký tự  
        # 3. Không nằm trong REMOVE_WORDS
        if (word.isdigit() or 
            len(word) >= 2 or 
...
def tach_phanchinh(text):
    ...
    SIMPLIFIED: Direct removal since mapping data doesn't contain admin units
